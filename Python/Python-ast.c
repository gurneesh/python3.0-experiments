...
static int init_types(void)
{
        ...
        Unless_type = make_type("Unless", stmt_type, Unless_fields, 2);
        if (!Unless_type) return 0;
...
stmt_ty
Unless(expr_ty test, asdl_seq * body, int lineno, int col_offset, PyArena
       *arena)
{
        stmt_ty p;
        if (!test) {
                PyErr_SetString(PyExc_ValueError,
                                "field test is required for Unless");
                return NULL;
        }
        p = (stmt_ty)PyArena_Malloc(arena, sizeof(*p));
        if (!p)
...
ast2obj_stmt(void* _o)
{
        ...
        case While_kind:
                ...
                break;
        case Unless_kind:
                result = PyType_GenericNew(Unless_type, NULL, NULL);
                if (!result) goto failed;
                value = ast2obj_expr(o->v.Unless.test);
                if (!value) goto failed;
                if (PyObject_SetAttrString(result, "test", value) == -1)
                        goto failed;
                Py_DECREF(value);
                value = ast2obj_list(o->v.Unless.body, ast2obj_stmt);
                if (!value) goto failed;
                if (PyObject_SetAttrString(result, "body", value) == -1)
                        goto failed;
                Py_DECREF(value);
...
obj2ast_stmt(PyObject* obj, stmt_ty* out, PyArena* arena)
{
        ...
        if (PyObject_IsInstance(obj, (PyObject*)Unless_type)) {
                expr_ty test;
                asdl_seq* body;
                ...
                if (PyObject_HasAttrString(obj, "test")) {
                        ...
                        PyErr_SetString(PyExc_TypeError, "required field \"test\" missing from Unless");
                        return 1;
                }
                if (PyObject_HasAttrString(obj, "body")) {
                        int res;
                        ...
                        if (tmp == NULL) goto failed;
                        if (!PyList_Check(tmp)) {
                                PyErr_Format(PyExc_TypeError, "Unless field \"body\" must be a list, not a %.200s", tmp->ob_type->tp_name);
                                goto failed;
                        }
                        len = PyList_GET_SIZE(tmp);
                        body = asdl_seq_new(len, arena);
                ...
                *out = Unless(test, body, lineno, col_offset, arena);
                if (*out == NULL) goto failed;
                return 0;
        }
...
PyInit__ast(void)
{
        ...
        if (PyDict_SetItemString(d, "While", (PyObject*)While_type) < 0) return
            NULL;
        if (PyDict_SetItemString(d, "Unless", (PyObject*)Unless_type) < 0)
            return NULL;
